...
> Picture a bathtub. Water builds up in it over time. That buildup is a *stock* (also called a reservoir or accumulation). The faucet and drain are *flows*. They are what fills or empties it. Model weights are a stock. Training is the flow that fills them. Compute, electricity, and engineers are flows that determine how fast the stock builds.

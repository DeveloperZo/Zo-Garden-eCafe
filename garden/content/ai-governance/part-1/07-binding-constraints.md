...
*DIA-007 — Binding Constraints Status Map: six structural constraints, color-coded by current status. Amber = bypassed or weakening (Electricity Grid, Chip Supply). Blue = real mechanism, forming but not yet closed (Product Liability, EU AI Act). Red = permanently outside governance reach (Open-Weights Diffusion). Gray = structurally absent as a ceiling (Interpretability). Each card answers: what makes this a constraint, and what is it actually doing right now.*
